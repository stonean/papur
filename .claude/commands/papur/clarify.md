...
- Reference: §spec-requirements, §spec-lifecycle, §pipeline-boundaries, §text-first-artifacts (constitution loaded by `/papur:target` — do not re-read).
...
- Reference: §scenarios, §text-first-artifacts (constitution loaded by `/papur:target` — do not re-read).
