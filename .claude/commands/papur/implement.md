...
5. **Pre-done review gate** (primitive: `check-review-gate`, which evaluates all five checks below in this order and returns the verdict with the canonical blocked message). First confirm all `.md` files in the feature directory pass `npx markdownlint-cli2`; on failure, report the specific violations and do not propose the transition. Then confirm no scenario under the feature carries unresolved open questions: a scenario is an organizational split of the spec, so its questions are the spec's questions for completeness and the spec is not `done` while any remain (§spec-lifecycle; spec 046). On failure, halt with `blocked: {N} unresolved open question(s) in scenario(s) {names} — resolve them before completing`, followed by guidance to run `/papur:target {feature}/<scenario>` then `/papur:clarify` on each — or, when a question is deferred rather than undecided ("not now; revisit when X lands"), to record it in that scenario's `## Resolved Questions` with its trigger, since a deferral is a resolution with a condition and only `## Open Questions` entries count. This check is ordered ahead of the `review:` block because an unresolved design question is the more upstream defect: reviewing a design that is about to change wastes the review.

   Then confirm the spec declares no undischarged fold. A `folds-into:` key names an upstream spec this one's content still has to be folded into, and until that happens the fold is outstanding work — the same category as an unresolved scenario question, which is why it sits beside it rather than with the review checks. On failure, halt with `blocked: spec folds into {target} and the fold has not happened — resolve it before completing`, followed by guidance to run `/papur:fold`, which folds the spec into its target and retires the directory; a branch-scoped spec is **retired, not completed**, so this is not a check the transition can pass with the key still present. A spec that should stand on its own instead is renamed to the sequential `NNN-` form by hand with the key removed (§numbering, §spec-lifecycle). The check reads the key alone and never asks whether the target resolves: before the merge the target normally lives on the branch this tree forked from, so requiring it to resolve would refuse the normal case — existence is enforced at fold-back by `retire-feature`.
   ...
   Finally, confirm the recorded review still describes the current code. Diff the spec's **durable contracts** — `scenarios/*.md` and `data-model.md`, the artifacts a review actually reads — between `review.reviewed-against` and `HEAD`; `tasks.md`, `plan.md`, `review.md`, and `spec.md` are deliberately out of scope, and a contract that changed only by a repo-wide rename says what it said before and is not stale. When any remain, halt with: `blocked: review is stale — {N} durable contract(s) changed since reviewed-against {sha}: {paths}` followed by guidance to re-run `/papur:review` so the recorded verdict describes the current code. This check is ordered last because it is the weakest claim — the others say a review is missing or failing, this one says a passing review is out of date. Without it, a review that predates the code it nominally covers satisfies every other check. Otherwise, proceed.
