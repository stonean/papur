...
Use the session target from `.claude/papur-session.json`. If no session target is set, stop and tell the user to run `/papur:target` first.
...
- Reference: §bug-handling, §scenarios, §scenario-promotion (constitution loaded by `/papur:target` — do not re-read).

## Instructions

### Confirm target

1. Read `.claude/papur-session.json` to get the session target's feature.
...
1. **Does a spec exist for this behavior?**
   - If no — stop. Tell the user to create the spec first via `/papur:specify`, then come back to create the scenario.
...
Update `.claude/papur-session.json` to include the new scenario as the active target. Do not prompt for confirmation — the user explicitly asked to create this scenario.
...
- Suggested next step: `/papur:implement` to work on the new task
- **Stop here.** Do not begin implementation. The user will run `/papur:implement` when ready.
