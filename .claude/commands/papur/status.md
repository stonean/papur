...
1. The walker context already contains the session target's feature field when `.claude/papur-session.json` exists — the runtime exec subcommand seeds it from that file automatically. In the markdown-only path, read the session file directly to learn the current target (and any scenario it carries).
